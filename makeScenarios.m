...
%Epi:
R0=[1.45,1.7];%params(end-1);%[1.45,1.7];%Change
muTimes=[1*ones(5,1),[.5*ones(4,1);1]];%Divide incidence

%Vax:
vaxon=[1e6,107,157];%[0,1];%[1e6,107,157];%Administration %Change
%vaxparams=vaxIn;%Change (comment out next for 2009)
%
vaxparams=ones(5,3);
vaxparams(:,1)=[.2,.5,.2,.09,.01]'*2.5e7/3.282e8/7;%;%Rate 1
vaxparams(:,2)=[0,0,.25,.35,.4]'*2.5e7/3.282e8/7;%;%Rate 2
...
%%
%Scenario numbers:
vax1=[2,4,5,9,10,14,16,17,21,22];%[2,4,7,8];%[2,4,5,9,10,14,21,22];*Change
vax2=[3,6,7,11,12,1518,19,23,24];%0;%[3,6,7,11,12,15,23,24];%Change
sch1=[4,6,9,11,16,18,21,23];%[5,7];%[4,6,9,11,16,18,21,23];%Change
sch2=sch1+1;%[5,7];%sch1+1;
av=[8:12,20:24];%[3,4,7,8];%[8:12,20:24];%Change
%%
...
for i=1:numScen
    ...
    if i<=numScen/2
        ...
        sceni.R0=R0(2);%Change - 1 for 2009, 2 for hyp
        sceni.muTimes=muTimes(:,2);%Change - 1 for 2009, 2 for hyp
    end
    %Vax::
    if ismember(i,vax1)
        sceni.vaxon=vaxon(2);
...
F=cell(1,numScen);
Z=zeros(numScen,na);
xsto=xstoFull(40001:500:end,:);
lx=size(xsto,1);
for i=1:numScen
    Fi=zeros(numDays,20,lx);
    for j=1:lx
        [f,~,z2]=subPandemicSimulationVaxScenarios(NNbar,xsto(j,:),xdata,0,0,ydata,243,scenariosAll{i});%Change %params
