function [f,g]=heSimCovid19(pr,beta,tvec,Dvec,n,nbar,NNvec,phi1,phi2,seedvec,S0,tau)
%{
...
if solvetype==2
    ...
    for i=1:lt-1
        ...
        %}
        topen=1;%tvec(3);
        [tout,Sclass,Hclass,Dclass,DEcum,Rcum,Itot,y0]=integr8(pr,beta,nbar,NNfeed,D,phi1,phi2,seedvec,t0,tend,y0,topen);
...
%For plots:
f=toutAll;%(2:end);
g=Rout+DEout;%sum(Hout,2);%-diff(sum(Sout,2))./diff(toutAll);
%g=cumsum(Iout);
%g=sum(Dout,2);
end
