% Code to get age-specific inputs for the modelling (FOUR age groups only):
...
popns = mat(:,1,4);
tmp1 = mat(:,:,3)';                                                        % Dims: 1.Week 2.Age group

% Aggregate 3rd and 4th age groups
tmp2 = tmp1(:,[1,2,3,5]);
tmp2(:,3) = tmp1(:,3) + tmp1(:,4);
data = tmp2;
...
mults.mu = cell2mat(raw(a(1)+[2:5], b(1)))';                               % Mean for normal distribution on multipliers

% Aproximate sigma (standard deviation) using 2SD approximation
lohi = cell2mat(raw(a(1)+[2:5], [b(1)-1,b(1)+1]))';
...
% Age groups: <5, 5-17, 18-24,   65+ 
aggreg_inds = {1,  2:4,  5:13, 14:15};  

num_ages = length(aggreg_inds);
