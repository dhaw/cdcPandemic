function [c,z]=subDecisionSeverity(NNbar,xsto)%ydata,ydataNX)
...
for i=1:lx
    ...
    for j=1:floor((chainLength-burn)/inc)%burn+1:inc:length(xsto)
        jj=inc*(j-1)+1;
        [~,~,z1]=subPandemicSimulation(NNbar,xsto(jj,:),1,0,0,0,243);
        z1=sum(z1.*qmean);
        [~,~,z2]=subPandemicSimulation(NNbar,xsto(jj,:),1,0,0,0,1000);
        z2=sum(z2.*qmean);
        zhold(j,1)=z1;
        zhold(j,2)=z2;
    end
